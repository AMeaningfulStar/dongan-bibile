...
import { createCourse, updateCourse } from '@/features/admin/courses/actions'
...
type Props = {
  mode: 'create' | 'edit'
  courseId?: string
  initialValues?: Partial<CreateCourseInput>
}

const DEFAULT_VALUES: CreateCourseInput = {
  title: '',
  startDate: new Date(),
...
function toInputDateValue(d: unknown) {
...
function coerceDate(v: unknown): Date {
  ...
  // Firestore Timestamp(클라이언트 SDK) 대응 (toDate 존재)
...
export function CourseCreateForm({ mode, courseId, initialValues }: Props) {
  ...
  const form = useForm<CreateCourseInput>({
    ...
    defaultValues: {
      ...DEFAULT_VALUES,
      ...initialValues,
      startDate: coerceDate(initialValues?.startDate ?? DEFAULT_VALUES.startDate),
    },
    mode: 'onChange',
  })
  ...
  const onSubmit = (values: CreateCourseInput) => {
    startTransition(async () => {
      try {
        ...
        await updateCourse(courseId, { ...values, id: courseId })
      ...
      <form onSubmit={form.handleSubmit(onSubmit)} className="space-y-6">
        ...
        <div className="space-y-2">
          ...
          <Input
            id="startDate"
            type="date"
            value={toInputDateValue(form.watch('startDate'))}
            onChange={(e) => {
